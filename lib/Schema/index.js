"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _bluebird = require("bluebird");
...
Schema = /*#__PURE__*/function () {
  ...
  function Schema(schema, options) {_classCallCheck(this, Schema);this.schema = {};this.options = {};this.methods = {};this.statics = {};
    this.schema = schema || {};
  ...
  }_createClass(Schema, [{ key: "getSchemaDescription", value: function getSchemaDescription()
...

    {
      ...
      return ret;
    } }, { key: "getSchemaUDTDescription", value: function () {var _getSchemaUDTDescription = (0, _bluebird.coroutine)(function* ()
      {
        const desc = this.getSchemaDescription();

        let fields = {};

        const traverse = /*#__PURE__*/function () {var _ref = (0, _bluebird.coroutine)(function* (sf, _k, _key, _subkeyarr) {
            ...
            if (sf[_k] && typeof sf[_k] === "object" && sf[_k].constructor === Object && !sf[_k].hasOwnProperty("virtual")) {
              if (sf[_k].hasOwnProperty("type")) {
                ...
                if (!type) {
                  const complexType = yield Schema.simpleComplexToCassandra(sf[_k].type, sf[_k].of);
                  if (!complexType) {
              ...
              } else
              {
                ...
                for (let i = 0; i < sf_karr.length; i++) {
                  ...
                  yield traverse(sf[_k], sf_k, `${_key}.${sf_k}`, subkeys);
                }

              }
            }
          });return function traverse(_x, _x2, _x3, _x4) {return _ref.apply(this, arguments);};}();

        const desc_karr = Object.keys(desc);
        for (let i = 0; i < desc_karr.length; i++) {
          const k = desc_karr[i];
          yield traverse(desc, k, k);
        }

        return fields;
      });function getSchemaUDTDescription() {return _getSchemaUDTDescription.apply(this, arguments);}return getSchemaUDTDescription;}() }, { key: "createOrGetUDT", value: function () {var _createOrGetUDT = (0, _bluebird.coroutine)(function* (
      _client) {
        ...
        const thisUDT = yield this.getSchemaUDTDescription();

        const udtBuilder = new _udt.default(_client);

        const existentUDT = yield new Promise((resolve, reject) => {
      ...
      });function createOrGetUDT(_x5) {return _createOrGetUDT.apply(this, arguments);}return createOrGetUDT;}() }, { key: "_addFlags", value: function _addFlags(
    ...
    } }, { key: "_addDefault", value: function _addDefault(
    sf, cf, thisschema) {
    ...
    } }, { key: "_addValidators", value: function _addValidators(
    sf, cf, thisschema) {
      const validatorFns = [];
    ...
    } }, { key: "_addGetterSetter", value: function _addGetterSetter(
    sf, cf, thisschema) {
    ...
    } }, { key: "_addTransformations", value: function _addTransformations(
    sf, k, before_save, before_update) {
      if (sf.lowercase) {
    ...
    } }, { key: "_addKeysIndexes", value: function _addKeysIndexes(
    sf, k, primary_key, clustering_key, secondary_index, unique_index) {
      if (sf.primary_key) {
        primary_key.push(k);
      }
      ...
      if (typeof sf.secondary_index === "object") {
        secondary_index.push(_objectSpread2({}, sf.secondary_index, { indexed: [k] }));
      }
      if (sf.unique === true) {
        unique_index.push(k);
      }
    } }, { key: "toExpressCassandra", value: function () {var _toExpressCassandra = (0, _bluebird.coroutine)(function* (

      _client, fieldsOnly) {var _this = this;
        ...
        for (let i = 0; i < schemaDesc_karr.length; i++) {
          ...
          const makeField = /*#__PURE__*/function () {var _ref2 = (0, _bluebird.coroutine)(function* (field, key) {
              const sfPrimitiveType = Schema.primitiveToCassandra(field.type);
              const sfSimpleComplexType = yield Schema.simpleComplexToCassandra(field.type, field.of);
              ...
              _this._addFlags(field, cf);
              _this._addDefault(field, cf, thisschema);
              _this._addValidators(field, cf, thisschema);
              _this._addGetterSetter(field, cf, thisschema);
              _this._addTransformations(field, key, before_save, before_update);
              _this._addKeysIndexes(field, key, primary_key, clustering_key, secondary_index, unique_index);

              return cf;
            });return function makeField(_x8, _x9) {return _ref2.apply(this, arguments);};}();

          const traverse = /*#__PURE__*/function () {var _ref3 = (0, _bluebird.coroutine)(function* (sf, _k, _key, _subkeyarr) {

              if (sf[_k] && typeof sf[_k] === "object" && sf[_k].constructor === Object) {
                if (sf[_k].hasOwnProperty("type")) {
                  const cf = yield makeField(sf[_k], _key);
                  fields[_key] = cf;
              ...
              sf[_k].constructor === Array || sf[_k].constructor === Set))
              {
                ...
                if (sarr.length === 1 && typeof sarr[0] === "object" && sarr[0].constructor === Schema) {
                  const exprCassObj = yield sarr[0].toExpressCassandra(_client);

                  fields[_key] = {
                    type: sf[_k].constructor === Array ? "list" : "set",
                    typeDef: `<${yield sarr[0].createOrGetUDT(_client)}>`,
                    rule: {
                      validator: value => {
                        const vals = [...value] || [];
            ...
            });return function traverse(_x10, _x11, _x12, _x13) {return _ref3.apply(this, arguments);};}();

          yield traverse(schemaDesc, k, k);
        }
      ...
      });function toExpressCassandra(_x6, _x7) {return _toExpressCassandra.apply(this, arguments);}return toExpressCassandra;}() }], [{ key: "primitiveToCassandra", value: function primitiveToCassandra(primitiveType) {if (typeof primitiveType === "string" && ["set", "list", "map"].indexOf(primitiveType) === -1) {return primitiveType;}switch (primitiveType) {case String:return "text";case Number:return "float";case Boolean:return "boolean";case Date:return "timestamp";case Buffer:return "blob";}return null;} }, { key: "simpleComplexToCassandra", value: function () {var _simpleComplexToCassandra = (0, _bluebird.coroutine)(function* (simpleType, ofType) {if (simpleType.__proto__ && simpleType.__proto__.name === "Proxy" && simpleType.name === "Map") {if (!Array.isArray(ofType) || !ofType.length === 2) {throw new Error("When using 'Map' as field-type, 'of'-Property of field must be set with an Array containing the exact two Types for map<type1, type2>.");}return new Promise( /*#__PURE__*/function () {var _ref4 = (0, _bluebird.coroutine)(function* (resolve, reject) {const keyType = ofType[0];const valType = ofType[1];const makeTypeDef = str => `<${str}>`;if (valType.constructor && valType.constructor.name === "Schema") {const udt = yield valType.createOrGetUDT();resolve({ type: "map", typeDef: makeTypeDef(`${Schema.primitiveToCassandra(keyType)},${udt}`) });} else {const primVal = Schema.primitiveToCassandra(valType);if (primVal) {resolve({ type: "map", typeDef: makeTypeDef(`${Schema.primitiveToCassandra(keyType)},${primVal}`) });} else {if (Array.isArray(valType)) {const scomValOfType = valType[0];const simpCompType = yield Schema.simpleComplexToCassandra(Array, scomValOfType);if (simpCompType) {resolve({ type: "map", typeDef: makeTypeDef(`${Schema.primitiveToCassandra(keyType)},frozen<${simpCompType.type}${simpCompType.typeDef}>`) });} else {reject();}} else {const simpCompVal = yield Schema.simpleComplexToCassandra(valType);if (simpCompVal) {resolve({ type: "map", typeDef: makeTypeDef(`${Schema.primitiveToCassandra(keyType)},${simpCompVal}`) });} else {reject();}}}}});return function (_x16, _x17) {return _ref4.apply(this, arguments);};}());}if (typeof ofType !== "string") {switch (ofType) {case String:case Number:case Boolean:case Date:case Buffer:break;default:return null;}}switch (simpleType) {case Array:case "list":return { type: "list", typeDef: `<${Schema.primitiveToCassandra(ofType)}>` };case Set:case "set":return { type: "set", typeDef: `<${Schema.primitiveToCassandra(ofType)}>` };case "map":return { type: "map", typeDef: `<${Schema.primitiveToCassandra(ofType)}>` };}return null;});function simpleComplexToCassandra(_x14, _x15) {return _simpleComplexToCassandra.apply(this, arguments);}return simpleComplexToCassandra;}() }]);return Schema;}();let



SchemaJSONBFieldProxy = /*#__PURE__*/function (_Proxy) {_inherits(SchemaJSONBFieldProxy, _Proxy);

  function SchemaJSONBFieldProxy(obj) {_classCallCheck(this, SchemaJSONBFieldProxy);return _possibleConstructorReturn(this, _getPrototypeOf(SchemaJSONBFieldProxy).call(this,
    (obj => {
  ...
  }return SchemaJSONBFieldProxy;}(Proxy);var _default =
