...
class Schema {
  ...
  static async simpleComplexToCassandra(simpleType, ofType, _client) {
          ...
          } else
          {
            if (Array.isArray(valType)) {
              if (typeof valType[0] === "object" && valType[0].hasOwnProperty("ref") && typeof valType[0].ref === "string") {
                ...
                if (refschema) {
                  let refkey = [];
                  if (valType[0].hasOwnProperty("refProp") && valType[0].refProp) {
                    refkey = [].concat(...[valType[0].refProp]);
                  } else
                  {
            ...
            if (typeof valType === "object" && valType.hasOwnProperty("ref") && typeof valType.ref === "string") {
              ...
              if (refschema) {
                let refkey = [];
                if (valType.hasOwnProperty("refProp") && valType.refProp) {
                  refkey = [].concat(...[valType.refProp]);
                } else
                {
                  refkey = [].concat(...(Array.isArray(refschema.options.key) ? refschema.options.key : [refschema.options.key]));
                }
      ...
      const traverse = async (sf, _k, _key, _subkeyarr) => {
        ...
        if (sf[_k] && typeof sf[_k] === "object" && sf[_k].constructor === Object) {
          ...
          if (sf[_k].hasOwnProperty("ref") && typeof sf[_k].ref === "string") {
            ...
            if (refschema) {
              let refkey = [];
              if (sf[_k].hasOwnProperty("refProp") && sf[_k].refProp) {
                refkey = [].concat(...[sf[_k].refProp]);
              } else
              {
                refkey = [].concat(...(Array.isArray(refschema.options.key) ? refschema.options.key : [refschema.options.key]));
              }
              ...
              } else
              {
                const findRefType = _refschema => {
                  const refschemaRefKeyProp = (0, _get.default)(_refschema.schema, refkey[0]);
                  if (
                  refschemaRefKeyProp &&
              ...
              fields[_key].refProp = sf[_k].refProp;
        ...
        sf[_k].constructor === Array || sf[_k].constructor === Set))
        {
          ...
          sarr[0].hasOwnProperty("ref") && typeof sarr[0].ref === "string")
          {
            ...
            if (refschema) {
              let refkey = [];
              if (sarr[0].hasOwnProperty("refProp") && sarr[0].refProp) {
                refkey = [].concat(...[sarr[0].refProp]);
              } else
              {
                refkey = [].concat(...(Array.isArray(refschema.options.key) ? refschema.options.key : [refschema.options.key]));
              }
              ...
              fields[_key].refProp = sarr[0].refProp;
