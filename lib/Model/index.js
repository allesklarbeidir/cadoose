      ...
      if (ref) {
        ...
        if (refschema) {
          ...
          if (Array.isArray(instProp)) {
            ...
            const fetchedRefs = await cadoose().models[ref].findAsync(_objectSpread2({}, queryObject));
            (0, _set.default)(this._instance, prop, fetchedRefs);
          } else
          {
            const queryObject = makeQueryObject(instProp);

            const fetchedRef = await cadoose().models[ref].findOneAsync(_objectSpread2({}, queryObject));
