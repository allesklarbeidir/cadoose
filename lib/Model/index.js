...
class Model {
  ...
  constructor(expressCassandraModel, schemaSchema) {this._model = {};this._name = "";this._schema = {};this.Model = null;this.
    ...
    syncDB = callback => {
      ...
      const afterDBCreate = (err1, crt_res, _nextcallback) => {
        if (err1) {
          _nextcallback(err1);
          return;
        }
        ...
        if (Array.isArray(modelSchema.indexes) || Array.isArray(modelSchema.ycql_indexes)) {
          tableBuilder.createIndexesAsync = _bluebird.default.promisify((indexes, _nextcallback) => {
            if (indexes.map(idx => typeof idx === "string").filter(Boolean).length === indexes.length) {
              return tableBuilder.create_indexes(indexes, _nextcallback);
            } else
            {
        ...
        if (Array.isArray(modelSchema.unique)) {
          tableBuilder.createUniqueIndexesAsync = _bluebird.default.promisify((unique, _nextcallback) => {
        ...
        if (!modelSchema.original_cassandra_indexes) {
          modelSchema.original_cassandra_indexes = [...modelSchema.indexes];
        } else
        {
          modelSchema.indexes = [...modelSchema.original_cassandra_indexes];
        }

        modelSchema.indexes = [
        ...modelSchema.indexes.map(idx => [...new Set([idx, ...modelSchema.key])].join(",")),
        ...[].concat(
        ...(modelSchema.ycql_indexes || []).map(idx => typeof idx === "object" ?
        Array.isArray(idx.indexed) ?
        [...new Set([...idx.indexed, ...modelSchema.key])].join(",") :
        [...new Set([idx.indexed, ...modelSchema.key])].join(",") :
        [...new Set([idx, ...modelSchema.key])].join(","))),


        ...[].concat(
        ...(modelSchema.unique || []).map(idx => typeof idx === "object" ?
        Array.isArray(idx.indexed) ? idx.indexed.join(",") : idx :
        idx))];
      ...
      this._create_table(modelSchema, (_err, _res) => {
        afterDBCreate(_err, _res, () => {
...
class ModelInstance {
  ...
  constructor(uid, instance, modelPrx) {this._uid = null;this._instance = null;this.
    ...
    populate = async prop => {
      ...
      const refField = model._schema.fields[prop] && model._schema.fields[prop] || null;
      if (refField.ref) {
        const ref = refField.ref;
        const refschema = ref && cadoose().schemas[ref] || null;
        if (refschema) {
          let refkey = [];
          if (refField.hasOwnProperty("refProp") && refField.refProp) {
            refkey = [].concat(...[refField.refProp]);
          } else
          {
            refkey = [].concat(...(Array.isArray(refschema.options.key) ? refschema.options.key : [refschema.options.key]));
          }
            ...
            if (refkey.length === 1) {
              ...
              if (Array.isArray(Object.values(instProp)[0])) {
                ...
                for (let i = 0; i < instPropKeys.length; i++) {
                  ...
                  (0, _set.default)(this._instance, `${prop}.${instPropKey}`, fetchedRefs || []);
              ...
              } else
              {
                ...
                Object.keys(instProp).forEach(_k => {
                  const farr = (fetchedRefs || []).filter(fr => (0, _get.default)(fr, refkey[0]) === instProp[_k]);
                  (0, _set.default)(this._instance, `${prop}.${_k}`, farr.length > 0 ? farr[0] : null);
                });
              }
  ...
  Object.keys(s).forEach(k => {
    ...
    if ((0, _get.default)(instanceValues, k, null) !== null) {
      ...
      if (s[k] && s[k].hasOwnProperty("ref") && !fromDB) {
        ...
        if (refschema) {
          let refkey = [];
          if (s[k].hasOwnProperty("refProp") && s[k].refProp) {
            refkey = [].concat(...[s[k].refProp]);
          } else
          {
            refkey = [].concat(...(Array.isArray(refschema.options.key) ? refschema.options.key : [refschema.options.key]));
          }
          ...
          } else
          {
            ...
            if (isMapWithRefs) {
              Object.keys((0, _get.default)(instanceValues, k)).forEach(_k => {
                const prop = (0, _get.default)(instanceValues, `${k}.${_k}`);
                if (typeof reftype === "function") {
                  if (Array.isArray(prop)) {
                    instanceValues[k][_k] = prop.map(pref => {
                      return reftype((0, _get.default)(pref, refkey[0]));
                    });
                  } else
                  {
                    instanceValues[k][_k] = reftype((0, _get.default)(instanceValues, `${k}.${_k}.${refkey[0]}`));
                  }
                } else
                {
                  if (Array.isArray(prop)) {
                    instanceValues[k][_k] = prop.map(pref => {
                      return (0, _get.default)(pref, refkey[0]);
                    });
                  } else
                  {
                    instanceValues[k][_k] = (0, _get.default)(instanceValues, `${k}.${_k}.${refkey[0]}`);
                  }
                }
              });
            } else
            if (Array.isArray((0, _get.default)(instanceValues, k))) {
              ...
              instanceValues[k] = (0, _get.default)(instanceValues, k).map(instValRef => {
                if (typeof reftype === "function") {
                  return reftype((0, _get.default)(instValRef, refkey[0]));
                } else
                {
                  return (0, _get.default)(instValRef, refkey[0]);
                }
              });
            ...
            {
              if (typeof reftype === "function") {
                instanceValues[k] = reftype((0, _get.default)(instanceValues, `${k}.${refkey[0]}`));
              } else
              {
                instanceValues[k] = (0, _get.default)(instanceValues, `${k}.${refkey[0]}`);
              }
